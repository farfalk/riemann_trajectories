...
# reference:
# http://www.martinos.org/mne/stable/generated/mne.datasets.eegbci.load_data.html
...
# saves in "raws" the content of the edfs
...
ax.set_xlim3d([min([min(x_c3cz[0, :]), min(x_c3c4[0, :]), min(x_czc4[0, :])]),
               max([max(x_c3cz[0, :]), max(x_c3c4[0, :]), max(x_czc4[0, :])])])
ax.set_ylim3d([min([min(x_c3cz[1, :]), min(x_c3c4[1, :]), min(x_czc4[1, :])]),
               max([max(x_c3cz[1, :]), max(x_c3c4[1, :]), max(x_czc4[1, :])])])
ax.set_zlim3d([min([min(x_c3cz[2, :]), min(x_c3c4[2, :]), min(x_czc4[2, :])]),
               max([max(x_c3cz[2, :]), max(x_c3c4[2, :]), max(x_czc4[2, :])])])
...
# TODO introduce the nice visualization ideas used in the matlab prototypes.
